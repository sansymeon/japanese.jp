#!/usr/bin/env python3
"""
KML painterly prompt generation v3 (production).
...
Writes (default output root: kml/data/kanji/painterly_prompts_v3/):
  - prompts/lesson_XX/{slug}_{variant}_{lesson:02d}.txt
  - combined/all_prompts_combined.txt
  - metadata/painterly_prompts_v3_catalog.json

Missing mask files are logged; generation never stops for missing masks.
"""
...
DEFAULT_OUTPUT_ROOT = KML_DIR / "data" / "kanji" / "painterly_prompts_v3"
...
def resolve_mask_path(base_kanji_dir: Path, lesson: int, slug: str) -> tuple[Path | None, str]:
    """
    Return (existing_path_or_None, reference_string_for_prompt).
    """
    candidates = [
...
def variant_section(variant_id: str, variant_lookup: dict[str, dict[str, Any]]) -> str:
    ...
    # Dedicated philosophy blocks: materially distinct systems (not tiny tweaks).
    philosophy = {
        "full_impasto": """
...
def build_prompt_text(
    *,
    ...
    prompt = f"""
KML Painterly Prompt (schema {PROMPT_VERSION})
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Generate KML painterly prompts v3.")
    ...
    log = logging.getLogger("painterly_v3")
    ...
    catalog_path = meta_dir / "painterly_prompts_v3_catalog.json"
    ...
    if not args.dry_run:
        ...
        missing_path = meta_dir / "missing_masks_v3.txt"
    ...
    if missing_masks:
        # Keep logs readable: show first 30 unique
