"""Grade 2 (elementary) Jōyō kanji for the cheerful soundtrack series."""
...
def load_grade_2_kanji() -> list[KanjiEntry]:
    """Grade-2 Jōyō kanji in school joyo_index order."""
    rows: list[KanjiEntry] = []
    with MASTER_CSV.open(encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row.get("category") != "joyo" or row.get("grade") != "2":
                continue
            rows.append(
                KanjiEntry(
                    kanji=row["kanji"],
