#!/usr/bin/env python3
"""Build exhibition/lesson_37_study.json — original order, Gallery Seal Ending."""
...
STUDY_PATH = ROOT / "collections" / "lesson_37_study.json"
OUT_PATH = ROOT / "exhibition" / "lesson_37_study.json"
...
def build() -> dict:
    if not STUDY_PATH.is_file():
        raise FileNotFoundError(
            f"Missing {STUDY_PATH}. Run: python3 scripts/build_lesson_37_study.py"
        )
    study = json.loads(STUDY_PATH.read_text(encoding="utf-8"))
    config = exhibition_study_config(
        lesson=37,
        title="KML Ambient Study — Lesson 37 (Exhibition)",
        notes=(
