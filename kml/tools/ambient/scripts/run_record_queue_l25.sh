#!/usr/bin/env bash
# Recording queue: Lesson 25 — remaining stages (components already recorded).
#
# Safeguards (match the L23–24 queue):
...
#   nohup bash scripts/run_record_queue_l25.sh \
#     > /tmp/record_queue_l25.nohup.out 2>&1 &
set -uo pipefail
cd "$(dirname "$0")/.."
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
export PYTHONUNBUFFERED=1
LOG=collections/record_queue_l25.log
...

{
  ...
  echo "Lesson 25 × 7 stages (existing MP4s skipped; components expected present)"
