#!/usr/bin/env python3
"""Build Grade 2 Compounds — school edition (8 parts × 20 kanji, last part 21).
...
from grade_2_anchor_compounds_data import ordered_anchor_entries  # noqa: E402
from grade2_compounds_school_common import (  # noqa: E402
    COLLECTION_PREFIX,
    CONTENT_TYPE,
    DEFAULT_EXHIBITION,
    EDITION,
    ...
    format_duration,
    kanji_color,
    part_and_index_for_display_order,
    plan_batches,
    probe_duration_ms,
)
...
def scene_from_entry(entry: dict, *, part: int, index_in_part: int) -> dict:
    ...
    return {
        "id": f"G2_compounds_{order:02d}_{kanji}",
        "kanji": kanji,
        "anchor": anchor,
        "meta": meta,
    }
...
def build_full_collection(*, confirmation_mode: str = "stacked") -> dict:
    ...
    scenes = [
        scene_from_entry(
            entry,
            part=part_and_index_for_display_order(entry.get("displayOrder", i + 1))[0],
            index_in_part=part_and_index_for_display_order(entry.get("displayOrder", i + 1))[1],
        )
        for i, entry in enumerate(entries)
    ]
...
def main() -> int:
    ...
    parser.add_argument("--all", action="store_true", help="Build all seeded parts")
    parser.add_argument(
        "--full",
        action="store_true",
        help="Also build monolithic collection (grade_2_compounds_school)",
    )
    parser.add_argument("--plan", action="store_true", help="Print batch plan for seeded kanji")
    parser.add_argument(
        "--mode",
    ...
    if args.plan:
        print(f"Grade 2 compounds school batch plan ({len(all_entries)} kanji seeded)")
        print(f"  layout: up to {PART_COUNT} parts × {KANJI_PER_PART} kanji (~{per_card}ms/card)")
    ...
    if args.all or args.part:
        ...
        for part in parts_to_build:
            ...
            if not match:
                print(f"No batch for part {part} (not seeded yet).", file=sys.stderr)
                return 1
            _, start, end = match
            chunk = all_entries[start:end]
