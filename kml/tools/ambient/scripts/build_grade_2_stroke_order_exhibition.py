#!/usr/bin/env python3
"""Build Grade 2 Stroke Order exhibitions (8 parts: 7 × 20 + 1 × 21 kanji)."""
...
from grade2_stroke_order_common import (  # noqa: E402
    DEFAULT_EXHIBITION,
    KANJI_PER_PART,
    PART_COUNT,
    SERIES_ID,
...
def build_collection(part: int, entries: list) -> dict:
    ...
    return {
        ...
        "display": {
            "loop": False,
            "hideChrome": True,
            "family": "grade2StrokeOrder",
            "showKeyword": False,
            "showKanji": False,
            "showEnglish": False,
            "exhibitProfile": "grade2StrokeOrder",
        },
        "meta": {
            "series": SERIES_ID,
            "scope": SERIES_SCOPE,
            "part": part,
            "stage": "grade2StrokeOrder",
...
def main() -> int:
    ...
    parser.add_argument("--plan", action="store_true", help="Print batch plan for all 161 kanji")
    args = parser.parse_args()

    all_entries = load_grade_2_kanji()
    timing = dict(DEFAULT_EXHIBITION)
    batches = plan_batches(all_entries, timing)

    if args.plan:
        print(f"Grade 2 stroke order batch plan ({len(all_entries)} kanji)")
        print(f"  layout: {PART_COUNT - 1} × {KANJI_PER_PART} + 1 × 21")
