...
Uses exhibition/lesson_XX_foundations collections (Gallery Seal Ending, no loop).
...
Example:
  ...
  python3 scripts/record_foundations_exhibition.py --lessons 40
"""
...
OUTPUT_DIR = ROOT / "foundations_exhibitions"
...
def load_exhibition_collection(lesson: int) -> dict:
    path = ROOT / "exhibition" / f"lesson_{lesson}_foundations.json"
    if not path.is_file():
...
def record_lesson(*, lesson: int, port: int, output_dir: Path) -> Path:
    ...
    collection_name = f"lesson_{lesson}_foundations"
    url = f"http://127.0.0.1:{port}/index.html?collection={collection_name}&capture=1"
    out_path = output_dir / f"lesson_{lesson}_foundations.mp4"
