"""Grade 6 gojūon ordering, deep pigments, and part boundaries."""
...
from grade4_gojuon import HIRAGANA, KATAKANA, gojuon_key
from grade5_gojuon import PIGMENT_HEX as G5_PIGMENT_HEX
from grade_palette import mature_grade6

REPO = Path(__file__).resolve().parents[3]
MASTER_CSV = REPO / "data/kanji/kanji_master.csv"

PIGMENT_HEX: dict[str, str] = {k: mature_grade6(v) for k, v in G5_PIGMENT_HEX.items()}

# Inclusive section ranges per part (gojūon order).
# Parts 1, 2 & 4: grade_6_1 (~6:22 bed). Part 3: grade_6_2 (~6:12 bed).
...
def load_grade_6_sections() -> list[tuple[str, list[KanjiEntry]]]:
    """All grade-6 joyo kanji grouped by gojūon section, ordered."""
    groups: dict[str, list[KanjiEntry]] = defaultdict(list)
    with MASTER_CSV.open(encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row.get("category") != "joyo" or row.get("grade") != "6":
                continue
            key = gojuon_key(row)
            groups[key].append(
                KanjiEntry(
...
def sections_for_part(part: int) -> list[tuple[str, list[KanjiEntry]]]:
    all_sections = load_grade_6_sections()
