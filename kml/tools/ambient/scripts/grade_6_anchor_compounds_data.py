"""Grade 6 anchor compound seed data — school edition.

Source: collections/grade_6/grade_6_jukugo_list.json (10 parts; 20×9 + 11).
"""
...
JUKUGO_LIST = ROOT / "collections" / "grade_6" / "grade_6_jukugo_list.json"
...
def _load_jukugo_doc() -> dict[str, Any]:
    if not JUKUGO_LIST.is_file():
        raise FileNotFoundError(f"Missing Grade 6 jukugo list: {JUKUGO_LIST}")
...
def _entries_from_jukugo_doc(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    # Grade 5/6 list shape: {"jukugo": {"part1": [[kanji, word, reading], ...], ...}}
...
GRADE_6_ANCHOR_SEED: list[dict[str, Any]] = _entries_from_jukugo_doc(_load_jukugo_doc())

ANCHOR_BY_KANJI: dict[str, dict] = {
    entry["kanji"]: entry for entry in GRADE_6_ANCHOR_SEED
}


def ordered_anchor_entries() -> list[dict[str, Any]]:
    """Anchors in displayOrder (school jukugo batches)."""
    if not GRADE_6_ANCHOR_SEED:
        raise ValueError("No Grade 6 anchor compound entries seeded.")
    return sorted(GRADE_6_ANCHOR_SEED, key=lambda e: e["displayOrder"])
