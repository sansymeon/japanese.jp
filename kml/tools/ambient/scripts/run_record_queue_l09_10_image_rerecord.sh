#!/usr/bin/env bash
# Overnight image-stage re-record for Lessons 9–10 (updated study images).
...
#   L9 study_version_1, L10 study_version_2) then record_lesson_foundations.
#
# Start with:
#   nohup bash scripts/run_record_queue_l09_10_image_rerecord.sh \
#     > /tmp/record_queue_l09_10_image.nohup.out 2>&1 &
set -euo pipefail
cd "$(dirname "$0")/.."
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
export PYTHONUNBUFFERED=1
mkdir -p extended_exhibitions collections/lesson_09 collections/lesson_10
LOG=extended_exhibitions/lesson_09_10_image_rerecord.log
...
record_lesson() {
  ...
  echo "Updated images since prior recordings: L9 anxiety/faint/fathom/fire/hedge/inflammation/jade/level/pressure/seal/soil/spit/time; L10 black/black_ink/bury/carp/den/esteem/guard/point/quantity/same/thousandth"
...
echo "=== Lessons 9–10 image re-record started $(date -Iseconds) ==="
...
echo "=== Lessons 9–10 image re-record finished $(date -Iseconds) ==="
