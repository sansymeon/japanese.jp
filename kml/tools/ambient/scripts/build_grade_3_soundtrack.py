#!/usr/bin/env python3
"""Build Grade 3 Kanji Soundtrack collections (joyo order, cheerful edition)."""
...
from grade3_kanji_common import (  # noqa: E402
    CONTENT_TAIL_PAD_MS,
    DEFAULT_EXHIBITION,
    PART_KANJI_COUNT,
    PART_KANJI_OFFSET,
...
from grade3_musical_timing import (  # noqa: E402
    MS_PER_KANJI,
    fit_musical_entries,
    ...
    *,
    ...
    return {
        ...
        "display": {
            ...
            "showKanji": False,
            "showEnglish": False,
            "exhibitProfile": "grade3KanjiSoundtrack",
            "musicalTiming": True,
            "continuousFlow": True,
        },
        "meta": {
...
def main() -> int:
    ...
    entries = load_grade_3_kanji()
    if not entries:
        print("No grade-3 kanji found.", file=sys.stderr)
        return 1

    offset = args.offset if args.offset is not None else PART_KANJI_OFFSET.get(args.part, 0)
    size = args.kanji_per_part if args.kanji_per_part is not None else PART_KANJI_COUNT.get(args.part, 50)
