#!/usr/bin/env python3
"""Build Grade 1 Compounds — school edition (4 parts × 20 kanji).
...
A future ambient edition could reuse the same anchor data with edition: ambient.
"""
...
from grade_1_anchor_compounds_data import ordered_anchor_entries  # noqa: E402
from grade1_compounds_school_common import (  # noqa: E402
    COLLECTION_PREFIX,
    CONTENT_TYPE,
    DEFAULT_EXHIBITION,
    EDITION,
...
LEGACY_COLLECTION_ID = "grade_1_anchor_compounds_prototype"
...
def scene_from_entry(entry: dict, *, part: int, index_in_part: int) -> dict:
    ...
    return {
        "id": f"G1_compounds_{order:02d}_{kanji}",
        "kanji": kanji,
        "anchor": anchor,
        "meta": meta,
    }
...
def build_full_collection(*, confirmation_mode: str = "stacked") -> dict:
    """Monolithic 80-kanji collection (legacy QA / full run)."""
    entries = ordered_anchor_entries()
    timing = dict(DEFAULT_EXHIBITION)
    scenes = [
        scene_from_entry(entry, part=((entry.get("displayOrder", i + 1) - 1) // KANJI_PER_PART) + 1, index_in_part=i % KANJI_PER_PART)
        for i, entry in enumerate(entries)
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Also build monolithic 80-kanji collection (grade_1_compounds_school)",
    )
    parser.add_argument("--plan", action="store_true", help="Print batch plan for all 80 kanji")
    parser.add_argument(
        "--mode",
    ...
    batches = plan_batches(all_entries)
    timing = dict(DEFAULT_EXHIBITION)
    per_card = card_runtime_ms(timing)

    if args.plan:
        print(f"Grade 1 compounds school batch plan ({len(all_entries)} kanji)")
        print(f"  layout: {PART_COUNT} parts × {KANJI_PER_PART} kanji (~{per_card}ms/card)")
    ...
    if args.all or args.part:
        parts_to_build = (
            [args.part]
            if args.part
            else [part for part, _, _ in batches]
        )
        ...
        for part in parts_to_build:
            match = next((b for b in batches if b[0] == part), None)
            if not match:
                print(f"No batch for part {part}.", file=sys.stderr)
                return 1
            _, start, end = match
            chunk = all_entries[start:end]
            config = build_part_collection(part, chunk, confirmation_mode=args.mode)
            out_path = write_collection(config)
            print_part_summary(config, out_path)
            print()
            built_any = True

    if args.full or (not args.all and not args.part):
        config = build_full_collection(confirmation_mode=args.mode)
        out_path = write_collection(config)
        print_part_summary(config, out_path)

        legacy_path = write_collection_path(ROOT, LEGACY_COLLECTION_ID)
        legacy = dict(config)
        legacy["id"] = LEGACY_COLLECTION_ID
        legacy_path.write_text(
            json.dumps(legacy, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        print(f"Wrote legacy alias {legacy_path}")
        built_any = True
