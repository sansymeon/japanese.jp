#!/usr/bin/env bash
# After Kanji Components L6–10 finishes, rebuild + record Lessons 11–15.
#
# Outputs: collections/lesson_NN/components_lesson_NN.mp4
# Log:     collections/record_kanji_components_l11_15_after_l06_10.log
#
...
PRIOR_LOG=collections/record_kanji_components_l06_10_after_l01_05.log
LOG=collections/record_kanji_components_l11_15_after_l06_10.log
...
prior_recorders_idle() {
  ! real_proc 'scripts/record_lesson_components\.py --lesson (6|7|8|9|10)(\s|$)' \
    && ! real_proc 'scripts/run_kanji_components_l06_10_after_l01_05\.sh'
}

{
  echo "START $(date -Iseconds)"
  echo "Waiting for Kanji Components L6–10 DONE …"

  while true; do
    if prior_outputs_ready && rg -q '^DONE ' "$PRIOR_LOG" 2>/dev/null && prior_recorders_idle; then
      echo "Kanji Components L6–10 DONE spotted $(date -Iseconds)"
      break
    fi
    echo "waiting for Components L6–10… $(date -Iseconds)"
    sleep 60
  done
