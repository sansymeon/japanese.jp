#!/usr/bin/env python3
"""Record Rainbow Hiragana + Romaji MP4 via Playwright.

Output: collections/hiragana_song/hiragana_song_romaji.mp4

Uses exhibition.html?collection=hiragana_song_romaji
...
COLLECTION_ID = "hiragana_song_romaji"
...
def record(*, port: int) -> Path:
    ...
    # Self-paced animation (~5:37 authored) can outrun a shorter soundtrack.
    ...
    # The self-paced animation may outlast the soundtrack; -shortest trims to
    # audio, so pad silence to cover the full captured video.
    webm_s = probe_duration_seconds(webm)
    mp3_s = probe_duration_seconds(soundtrack)
    covered_s = soundtrack_start_ms / 1000.0 + mp3_s
