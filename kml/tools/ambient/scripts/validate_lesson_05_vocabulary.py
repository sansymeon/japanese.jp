#!/usr/bin/env python3
"""Editorial QA for Lesson 5 Vocabulary Exhibition."""
...
REPO = ROOT.parents[1]
STEPS_MOD = Path(__file__).resolve().parent / "lesson_05_vocabulary_steps.py"
LESSON_HTML = REPO / "contents/books/book_01/lessons/lesson_05.html"
COLLECTION = ROOT / "collections/lesson_05/lesson_05_vocabulary.json"
...
def verses_by_slug() -> dict[str, str]:
    ...
    from build_lesson_05_vocabulary_exhibition import parse_lesson_scenes

    return {
        s["meta"]["slug"]: s["verse"]["jpHtml"]
        for s in parse_lesson_scenes(5)
    }
...
def errors_for_steps(steps_by_slug: dict[str, list[dict]]) -> list[str]:
    ...
    for slug, steps in steps_by_slug.items():
        ...
        for i, step in enumerate(steps):
            ...
            if jp in BANNED_JP:
                errs.append(f"{label}: banned jp {jp!r}")
...
def main() -> int:
    ...
    all_errors: list[str] = []
    all_errors.extend(errors_for_html(lesson_html, label="lesson_05.html"))
