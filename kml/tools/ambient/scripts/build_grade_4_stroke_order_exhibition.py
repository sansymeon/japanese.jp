#!/usr/bin/env python3
"""Build Grade 4 Stroke Order exhibitions (8 parts; 25×6 + 26×2)."""
...
from grade_4_kanji import load_grade_4_kanji  # noqa: E402
from grade4_stroke_order_common import (  # noqa: E402
    DEFAULT_EXHIBITION,
    KANJI_PER_PART,
    PART_COUNT,
    SERIES_ID,
...
def build_collection(part: int, entries: list) -> dict:
    ...
    return {
        ...
        "display": {
            "loop": False,
            "hideChrome": True,
            "family": "grade4StrokeOrder",
            "showKeyword": False,
            "showKanji": False,
            "showEnglish": False,
            "exhibitProfile": "grade4StrokeOrder",
        },
        "meta": {
            "series": SERIES_ID,
            "scope": SERIES_SCOPE,
            "part": part,
            "stage": "grade4StrokeOrder",
...
def main() -> int:
    ...
    parser.add_argument("--plan", action="store_true", help="Print batch plan for all 202 kanji")
    args = parser.parse_args()

    all_entries = load_grade_4_kanji()
    timing = dict(DEFAULT_EXHIBITION)
    batches = plan_batches(all_entries, timing)

    if args.plan:
        soundtrack_ms = probe_duration_ms(ROOT / soundtrack_path_for_part(1)) or 0
        print(f"Grade 4 stroke order batch plan ({len(all_entries)} kanji)")
        print(f"  layout: 25×6 + 26×2 → {PART_COUNT} parts")
        if soundtrack_ms:
