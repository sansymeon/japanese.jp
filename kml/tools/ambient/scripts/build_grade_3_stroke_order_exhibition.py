#!/usr/bin/env python3
"""Build Grade 3 Stroke Order exhibitions (8 parts × 25 kanji)."""
...
from grade_3_kanji import load_grade_3_kanji  # noqa: E402
...
def build_collection(part: int, entries: list) -> dict:
    ...
    return {
        ...
        "display": {
            "loop": False,
            "hideChrome": True,
            "family": "grade3StrokeOrder",
            "showKeyword": False,
            "showKanji": False,
            "showEnglish": False,
            "exhibitProfile": "grade3StrokeOrder",
        },
        "meta": {
            "series": SERIES_ID,
            "scope": SERIES_SCOPE,
            "part": part,
            "stage": "grade3StrokeOrder",
...
def main() -> int:
    ...
    parser.add_argument("--plan", action="store_true", help="Print batch plan for all 200 kanji")
    args = parser.parse_args()

    all_entries = load_grade_3_kanji()
    timing = dict(DEFAULT_EXHIBITION)
    batches = plan_batches(all_entries, timing)

    if args.plan:
        print(f"Grade 3 stroke order batch plan ({len(all_entries)} kanji)")
        print(f"  layout: {PART_COUNT} × {KANJI_PER_PART}")
