#!/usr/bin/env python3
"""Build Grade 1 Stroke Order exhibitions (4 parts × 20 kanji)."""
...
def build_collection(part: int, entries: list) -> dict:
    ...
    return {
        ...
        "display": {
            "loop": False,
            "hideChrome": True,
            "family": "grade1StrokeOrder",
            "showKeyword": False,
            "showKanji": False,
            "showEnglish": False,
            "exhibitProfile": "grade1StrokeOrder",
        },
        "meta": {
            "series": SERIES_ID,
            "scope": SERIES_SCOPE,
            "part": part,
            "stage": "grade1StrokeOrder",
...
def main() -> int:
    ...
    all_entries = load_grade_1_kanji()
    timing = dict(DEFAULT_EXHIBITION)
    batches = plan_batches(all_entries, timing)

    if args.plan:
        print(f"Grade 1 stroke order batch plan ({len(all_entries)} kanji)")
        print(f"  layout: {PART_COUNT} parts × {KANJI_PER_PART} kanji")
