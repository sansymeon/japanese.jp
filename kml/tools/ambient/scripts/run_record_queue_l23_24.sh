#!/usr/bin/env bash
# Recording queue: Lessons 23–24 — all 7 stages (foundations through gallery).
#
# Safeguards (match the L21–22 queue):
...
#   nohup bash scripts/run_record_queue_l23_24.sh \
#     > /tmp/record_queue_l23_24.nohup.out 2>&1 &
set -uo pipefail
cd "$(dirname "$0")/.."
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
export PYTHONUNBUFFERED=1
LOG=collections/record_queue_l23_24.log
...

{
  ...
  echo "Lessons 23–24 × 7 stages (foundations, strokes, components, compounds, reading, vocabulary, gallery)"
