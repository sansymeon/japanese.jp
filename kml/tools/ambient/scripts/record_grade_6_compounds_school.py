#!/usr/bin/env python3
"""Record Grade 6 Compounds school edition MP4 via Playwright.

Output: collections/grade_6/grade_6_jukugo_{part}.mp4
"""
...
from grade6_compounds_school_common import PART_COUNT, collection_id  # noqa: E402
...
def record(*, part: int, port: int) -> Path:
    ...
    out_path = ROOT / "collections" / "grade_6" / f"grade_6_jukugo_{part}.mp4"
    tmp_dir = out_path.parent / f".tmp_grade_6_jukugo_{part}"
...
def main() -> int:
    ...
    if args.rebuild:
        subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_grade_6_compounds_school.py"), "--all"],
            check=True,
            cwd=ROOT,
        )
