#!/usr/bin/env python3
"""Build Lesson 2 Gallery — image and music only (final learning stage)."""
...
ASSETS = REPO / "assets"
LESSON = 2
COLLECTION_ID = "lesson_02_gallery"
...
# ~8 min Foundations bed (Lesson 2 → Study Version 1).
...
# Composition-aware camera (3–5% one-direction drift over each exhibit).
GALLERY_CAMERA_BY_SLUG: dict[str, dict] = {
    "prosperous": {"motion": "push-in"},
    "chant": {"motion": "drift-x"},
    "sparkle": {"motion": "push-in", "focus": "50% 42%"},
    "goods": {"motion": "drift-diagonal"},
    "spine": {"motion": "drift-y"},
    "early": {"motion": "rise"},
    "rising_sun": {"motion": "rise", "focus": "50% 32%"},
    "generation": {"motion": "push-in"},
    "stomach": {"motion": "drift-x", "focus": "50% 55%"},
    "dawn": {"motion": "rise", "focus": "50% 35%"},
    "gallbladder": {"motion": "drift-diagonal"},
    "span": {"motion": "drift-x"},
    "concave": {"motion": "push-in"},
    "convex": {"motion": "push-in", "focus": "50% 48%"},
    "olden_times": {"motion": "drift-diagonal"},
    "oneself": {"motion": "push-in"},
    "white": {"motion": "push-in", "focus": "50% 40%"},
    "hundred": {"motion": "drift-x"},
    "middle": {"motion": "drift-y"},
    "thousand": {"motion": "push-in"},
}
...
def gallery_exhibition_timing(scene_count: int, soundtrack_ms: int) -> dict:
    """Fit exhibit pacing to the Foundations bed; closing crest fades with soundtrack end."""
    transition_ms = 3500
    closing_lead_ms = sum(
        CLOSING_TIMING_MS[k]
        for k in (
...
def build() -> dict:
    ...
    return {
        ...
        "assetsBase": "../../assets",
        "id": COLLECTION_ID,
        "title": "KML Gallery — Lesson 2 (Foundations)",
        "notes": (
...
def main() -> int:
    ...
    audit = run_fill_safety_audit(COLLECTION_ID)
    return audit
