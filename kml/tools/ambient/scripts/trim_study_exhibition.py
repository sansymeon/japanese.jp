...
def chapter_timestamps(collection: dict, *, head_trim_s: float) -> list[str]:
    ...
    prep_first = t.get("studyKanjiGapMs", 350)
    prep_between = t.get("studyKanjiGapMs", 350)
...
def main() -> int:
    ...
    parser.add_argument("--input", type=Path, help="Source MP4 (default: study_exhibitions/)")
    ...
    src = args.input or ROOT / "study_exhibitions" / f"lesson_{args.lesson}_study.mp4"
    out = args.output or ROOT / "exhibition" / f"lesson_{args.lesson}_study.mp4"
    backup = src.parent / f"lesson_{args.lesson}_study_original.mp4"
    collection_path = ROOT / "exhibition" / f"lesson_{args.lesson}_study.json"
