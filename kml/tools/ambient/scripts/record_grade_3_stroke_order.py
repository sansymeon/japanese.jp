#!/usr/bin/env python3
"""Record Grade 3 Stroke Order exhibition MP4s via Playwright.

Output: collections/grade_3/grade_3_strokes_{part}.mp4
"""
...
from grade3_stroke_order_common import (  # noqa: E402
    PART_COUNT,
...
def apply_stroke_output_fade(path: Path) -> None:
    """Fade video/audio out from 9:15 and trim the file."""
...
def remux(*, part: int) -> Path:
    ...
    out_path = ROOT / "collections" / "grade_3" / f"grade_3_strokes_{part:02d}.mp4"
    if not out_path.is_file():
        raise FileNotFoundError(f"Missing video: {out_path}")

    tmp_dir = out_path.parent / f".tmp_grade_3_strokes_{part:02d}"
    ...
    print(f"  → {out_path} (fade @ 9:15, cut @ {STROKE_OUTPUT_DURATION_S // 60}:{STROKE_OUTPUT_DURATION_S % 60:02d})")
    return out_path
...
def main() -> int:
    ...
    parser.add_argument(
        "--fade",
        action="store_true",
        help="Apply 9:15 fade-out trim to existing MP4 (no remux)",
    )
    ...
    if args.rebuild:
        subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "build_grade_3_stroke_order_exhibition.py"),
                "--all",
            ],
            check=True,
            cwd=ROOT,
