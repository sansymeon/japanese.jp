#!/usr/bin/env python3
"""Record Grade 4 Compounds school edition MP4 via Playwright.

Output: collections/grade_4/grade_4_jukugo_{part}.mp4
"""
...
from grade4_compounds_school_common import PART_COUNT, collection_id  # noqa: E402
...
def record(*, part: int, port: int) -> Path:
    ...
    out_path = ROOT / "collections" / "grade_4" / f"grade_4_jukugo_{part}.mp4"
    tmp_dir = out_path.parent / f".tmp_grade_4_jukugo_{part}"
...
def main() -> int:
    ...
    if args.rebuild:
        subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_grade_4_compounds_school.py"), "--all"],
            check=True,
            cwd=ROOT,
        )
