#!/usr/bin/env bash
# After Kanji Components L21–25 finishes, rebuild + record Lessons 26–30.
#
# Outputs: collections/lesson_NN/components_lesson_NN.mp4
# Log:     collections/record_kanji_components_l26_30_after_l21_25.log
#
...
PRIOR_LOG=collections/record_kanji_components_l21_25_after_l16_20.log
LOG=collections/record_kanji_components_l26_30_after_l21_25.log
...
prior_recorders_idle() {
  ! real_proc 'scripts/record_lesson_components\.py --lesson (2[1-5])(\s|$)' \
    && ! real_proc 'scripts/run_kanji_components_l21_25_after_l16_20\.sh'
}

{
  echo "START $(date -Iseconds)"
  echo "Waiting for Kanji Components L21–25 DONE …"

  while true; do
    if prior_outputs_ready && rg -q '^DONE ' "$PRIOR_LOG" 2>/dev/null && prior_recorders_idle; then
      echo "Kanji Components L21–25 DONE spotted $(date -Iseconds)"
      break
    fi
    echo "waiting for Components L21–25… $(date -Iseconds)"
    sleep 60
  done
