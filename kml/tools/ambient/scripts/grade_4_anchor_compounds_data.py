"""Grade 4 anchor compound seed data — school edition.

Source: collections/grade_4/grade_4_jukugo_list.json (10 parts; 20×8 + 21×2).
"""
...
JUKUGO_LIST = ROOT / "collections" / "grade_4" / "grade_4_jukugo_list.json"
...
def _load_jukugo_doc() -> dict[str, Any]:
    if not JUKUGO_LIST.is_file():
        raise FileNotFoundError(f"Missing Grade 4 jukugo list: {JUKUGO_LIST}")
...
def _entries_from_jukugo_doc(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for part in doc.get("parts") or []:
        part_num = int(part.get("part") or 0)
...
def ordered_anchor_entries() -> list[dict[str, Any]]:
    ...
    if not GRADE_4_ANCHOR_SEED:
        raise ValueError("No Grade 4 anchor compound entries seeded.")
