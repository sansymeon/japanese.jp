#!/usr/bin/env bash
# Components-only: re-record Lesson 24 (updated decompositions) then record 25–30.
#
# Outputs: collections/lesson_NN/components_lesson_NN.mp4
# Log:     collections/record_kanji_components_l24_30.log
#
# Start with:
#   nohup bash scripts/run_kanji_components_l24_30.sh \
#     > /tmp/kanji_components_l24_30.nohup.out 2>&1 &
set -uo pipefail
cd "$(dirname "$0")/.."
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
export PYTHONUNBUFFERED=1

LOG=collections/record_kanji_components_l24_30.log
PY="$(pwd)/.venv/bin/python"
if [[ ! -x "$PY" ]]; then
  PY="$(command -v python3)"
fi

LESSONS=(24 25 26 27 28 29 30)
PORT_BASE=9824

{
  echo "START $(date -Iseconds)"
  echo "Components-only queue: L24 (re-record) → L25–30 (new)"

  echo "Ensuring self-hosted fonts…"
  bash scripts/fetch_noto_serif_jp_fonts.sh
  bash scripts/fetch_yuji_syuku_font.sh

  STAMP=$(date +%Y%m%d_%H%M)
  mkdir -p collections/_archive_components_pre_l24_30
  out24="collections/lesson_24/components_lesson_24.mp4"
  if [[ -f "$out24" ]]; then
    dest="collections/_archive_components_pre_l24_30/components_lesson_24_${STAMP}.mp4"
    mv "$out24" "$dest"
    echo "archived $out24 -> $dest"
  fi

  # Clear any stale tmp dirs for this range
  for n in "${LESSONS[@]}"; do
    nn=$(printf '%02d' "$n")
    rm -rf "collections/lesson_${nn}/.tmp_components_lesson_${nn}"
  done

  echo "==== Rebuild Kanji Components JSON from HTML (through L30) $(date -Iseconds) ===="
  "$PY" -u scripts/build_kanji_components.py --rebuild-html-db --max-lesson 30

  for n in "${LESSONS[@]}"; do
    nn=$(printf '%02d' "$n")
    out="collections/lesson_${nn}/components_lesson_${nn}.mp4"
    if [[ -f "$out" ]]; then
      echo "==== SKIP L$n components (exists: $out) $(date -Iseconds) ===="
      continue
    fi
    port=$((PORT_BASE + n))
    echo "==== LESSON $n / components  port=$port  $(date -Iseconds) ===="
    if "$PY" -u scripts/record_lesson_components.py --lesson "$n" --port "$port"; then
      ls -lh "$out" 2>/dev/null || echo "WARN: $out not produced"
    else
      echo "FAIL L$n / components (exit $?) — continuing queue"
    fi
  done

  echo "DONE $(date -Iseconds)"
} 2>&1 | tee -a "$LOG"
