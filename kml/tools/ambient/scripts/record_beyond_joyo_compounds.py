#!/usr/bin/env python3
"""Record Beyond Jōyō Kanji Compounds Part N MP4 via Playwright.

Usage:
  python scripts/record_beyond_joyo_compounds.py --part 1
  python scripts/record_beyond_joyo_compounds.py --part 1 --port 9011

Output: collections/beyond_joyo/beyond_joyo_compounds_{NN}.mp4
"""
...
def collection_id_for_part(part: int) -> str:
    return f"beyond_joyo_compounds_{part:02d}"
...
def record(*, part: int, port: int) -> Path:
    ...
    out_dir = ROOT / "collections" / "beyond_joyo"
...
def main() -> int:
    ...
    parser.add_argument("--part", type=int, required=True, help="Part number (1–19)")
    parser.add_argument("--port", type=int, default=None)
    args = parser.parse_args()
    if args.part < 1:
        raise SystemExit("--part must be >= 1")
    port = args.port if args.port is not None else 9010 + args.part
