#!/usr/bin/env python3
"""Record Lesson 1 study exhibition — mobile readability layout V2 test."""
...
def main() -> int:
    ...
    soundtrack = ROOT / (collection.get("soundtrack") or {}).get("main", "audio/study_version_3.mp3")
    if not soundtrack.is_file():
        print(f"Missing soundtrack: {soundtrack}", file=sys.stderr)
        return 1

    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    tmp_dir = OUTPUT.parent / ".tmp_lesson_01_mobile_v2"
    ...
    try:
        ...
        # Playwright sandbox sometimes blocks signals to the local server subprocess.
        # The MP4/mux usually succeeded already; we don't want a shutdown PermissionError
