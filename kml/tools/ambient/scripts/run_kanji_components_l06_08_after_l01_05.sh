#!/usr/bin/env bash
# After Kanji Components L1–5 (tonight rebuild) finishes, record Lessons 6–8.
# HTML for L6–8 was manually reviewed (Phase 1 H/V). Rebuild JSON from HTML,
# archive older MP4s, then record.
#
# Outputs: collections/lesson_NN/components_lesson_NN.mp4
# Log:     collections/record_kanji_components_l06_08_after_l01_05.log
#
# Start with:
#   nohup bash scripts/run_kanji_components_l06_08_after_l01_05.sh \
#     > /tmp/kanji_components_l06_08.nohup.out 2>&1 &
set -uo pipefail
cd "$(dirname "$0")/.."
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
export PYTHONUNBUFFERED=1

PRIOR_LOG=collections/record_kanji_components_l01_05_tonight.log
LOG=collections/record_kanji_components_l06_08_after_l01_05.log
...
prior_recorders_idle() {
  ! real_proc 'scripts/record_lesson_components\.py --lesson [1-5](\s|$)' \
    && ! real_proc 'record_kanji_components_l01_05'
}

{
  echo "START $(date -Iseconds)"
  echo "Waiting for Kanji Components L1–5 (tonight) DONE …"
  ...
  mkdir -p collections/_archive_components_pre_l06_08_rerecord
  echo "==== Archive prior L6–8 component MP4s $(date -Iseconds) ===="
  for n in "${LESSONS[@]}"; do
    nn=$(printf '%02d' "$n")
    out="collections/lesson_${nn}/components_lesson_${nn}.mp4"
    if [[ -f "$out" ]]; then
      dest="collections/_archive_components_pre_l06_08_rerecord/components_lesson_${nn}_${STAMP}.mp4"
      mv "$out" "$dest"
      echo "archived $out -> $dest"
    fi
  done

  echo "==== Rebuild Kanji Components JSON from HTML (through L8) $(date -Iseconds) ===="
  "$PY" -u scripts/build_kanji_components.py --rebuild-html-db --max-lesson 8
