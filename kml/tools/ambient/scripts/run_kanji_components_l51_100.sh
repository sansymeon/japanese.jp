#!/usr/bin/env bash
# Components-only: Lessons 51–100.
#
# Outputs: collections/lesson_NN/components_lesson_NN.mp4
# Log:     collections/record_kanji_components_l51_100.log
#
# Start with:
#   nohup bash scripts/run_kanji_components_l51_100.sh \
#     > /tmp/kanji_components_l51_100.nohup.out 2>&1 &
set -uo pipefail
cd "$(dirname "$0")/.."
...

{
  ...
  echo "Components-only queue: L51–100"
  ...
  echo "==== Rebuild Kanji Components JSON from HTML (through L100) $(date -Iseconds) ===="
  "$PY" -u scripts/build_kanji_components.py --rebuild-html-db --max-lesson 100
