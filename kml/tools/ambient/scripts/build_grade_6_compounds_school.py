#!/usr/bin/env python3
"""Build Grade 6 Compounds — school edition (10 parts from jukugo list).
...
from grade_6_anchor_compounds_data import ordered_anchor_entries, part_batches  # noqa: E402
from grade6_compounds_school_common import (  # noqa: E402
    COLLECTION_PREFIX,
    CONTENT_TYPE,
    DEFAULT_EXHIBITION,
    EDITION,
...
def scene_from_entry(entry: dict, *, part: int, index_in_part: int) -> dict:
    ...
    return {
        "id": f"G6_compounds_{order:03d}_{kanji}",
        "kanji": kanji,
        "anchor": anchor,
        "meta": meta,
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Also build monolithic collection (grade_6_compounds_school)",
    )
    ...
    if args.plan:
        print(f"Grade 6 compounds school batch plan ({len(all_entries)} kanji)")
