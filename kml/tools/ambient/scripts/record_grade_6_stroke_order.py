#!/usr/bin/env python3
"""Record Grade 6 Stroke Order exhibition MP4s via Playwright.

Output: collections/grade_6/grade_6_strokes_{part:02d}.mp4
"""
...
def remux(*, part: int) -> Path:
    ...
    out_path = ROOT / "collections" / "grade_6" / f"grade_6_strokes_{part:02d}.mp4"
    if not out_path.is_file():
        raise FileNotFoundError(f"Missing video: {out_path}")

    tmp_dir = out_path.parent / f".tmp_grade_6_strokes_{part:02d}"
...
def main() -> int:
    ...
    if args.rebuild:
        subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "build_grade_6_stroke_order_exhibition.py"),
                "--all",
            ],
            check=True,
            cwd=ROOT,
