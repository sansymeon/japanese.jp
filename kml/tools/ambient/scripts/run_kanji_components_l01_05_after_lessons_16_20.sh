#!/usr/bin/env bash
# After Lessons 16–20 Exhibition finishes, rebuild + record Kanji Components
# for Lessons 1–5 (approved vocabulary + New Component pacing).
#
# Outputs: collections/lesson_0N/components_lesson_0N.mp4
# Log:     collections/record_kanji_components_l01_05_after_16_20.log
set -uo pipefail
cd "$(dirname "$0")/.."
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
export PYTHONUNBUFFERED=1

PRIOR_LOG=extended_exhibitions/record_lessons_16_20_after_11_15.log
PRIOR_MP4=extended_exhibitions/lessons_16_20_prototype.mp4
PRIOR_ALT=extended_exhibitions/lessons_16_20_exhibition.mp4
LOG=collections/record_kanji_components_l01_05_after_16_20.log
PY="$(pwd)/.venv/bin/python"
if [[ ! -x "$PY" ]]; then
  PY="$(command -v python3)"
fi

LESSONS=(1 2 3 4 5)
PORT_BASE=9780

real_proc() {
  local pat=$1 line
  while IFS= read -r line; do
    case "$line" in
      *cursorsandbox*|*COMMAND_EXIT_CODE*|*AGENT_LOOP*|*pgrep*) continue ;;
    esac
    return 0
  done < <(pgrep -af "$pat" 2>/dev/null || true)
  return 1
}

prior_mp4_ready() {
  [[ -f "$PRIOR_MP4" ]] || [[ -f "$PRIOR_ALT" ]]
}

prior_recorders_idle() {
  ! real_proc 'scripts/record_japanese_reflections_exhibition\.py lessons_16_20' \
    && ! real_proc 'scripts/run_lessons_16_20_after_lessons_11_15\.sh'
}

{
  echo "START $(date -Iseconds)"
  echo "Waiting for Lessons 16–20 Exhibition DONE …"

  while true; do
    if prior_mp4_ready && rg -q '^DONE ' "$PRIOR_LOG" 2>/dev/null && prior_recorders_idle; then
      echo "Lessons 16–20 DONE spotted $(date -Iseconds)"
      break
    fi
    echo "waiting for Lessons 16–20… $(date -Iseconds)"
    sleep 60
  done

  echo "Ensuring self-hosted fonts…"
  bash scripts/fetch_noto_serif_jp_fonts.sh
  bash scripts/fetch_yuji_syuku_font.sh

  echo "==== Rebuild Kanji Components L1–5 JSON $(date -Iseconds) ===="
  "$PY" -u scripts/build_kanji_components.py

  for n in "${LESSONS[@]}"; do
    nn=$(printf '%02d' "$n")
    out="collections/lesson_${nn}/components_lesson_${nn}.mp4"
    if [[ -f "$out" ]]; then
      echo "==== SKIP L$n components (exists: $out) $(date -Iseconds) ===="
      continue
    fi
    port=$((PORT_BASE + n))
    echo "==== LESSON $n / components  port=$port  $(date -Iseconds) ===="
    if "$PY" -u scripts/record_lesson_components.py --lesson "$n" --port "$port"; then
      ls -lh "$out" 2>/dev/null || echo "WARN: $out not produced"
    else
      echo "FAIL L$n / components (exit $?) — continuing queue"
    fi
  done

  echo "DONE $(date -Iseconds)"
} 2>&1 | tee -a "$LOG"
