#!/usr/bin/env python3
"""Record Lesson 5 study exhibition — mobile-refine capture + external soundtrack mux."""
...
OUTPUT = ROOT / "foundations_exhibitions" / "ambient_study_lesson_5.mp4"
COLLECTION = ROOT / "exhibition" / "lesson_5_foundations.json"
PORT = 8778
...
def main() -> int:
    ...
    build = ROOT / "scripts" / "build_lesson_05_exhibition.py"
    if build.is_file():
        subprocess.run([sys.executable, str(build)], check=True, cwd=ROOT)

    if not COLLECTION.is_file():
        print(f"Missing {COLLECTION}. Run build_lesson_05_exhibition.py", file=sys.stderr)
        return 1
    ...
    soundtrack = ROOT / (collection.get("soundtrack") or {}).get("main", "audio/study_version_1_minus3db.mp3")
    if not soundtrack.is_file():
        print(f"Missing soundtrack: {soundtrack}", file=sys.stderr)
        return 1

    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    tmp_dir = OUTPUT.parent / ".tmp_lesson_05_mobile_refine"
    tmp_dir.mkdir(parents=True, exist_ok=True)

    url = (
        f"http://127.0.0.1:{PORT}/index.html"
        f"?collection=lesson_5_foundations&capture=1&typography=mobile-refine"
    )
    print("Recording Lesson 5 mobile-refine …")
    print(f"  URL: {url}")
