#!/usr/bin/env python3
"""Build Grade 5 Stroke Order exhibitions (8 parts; 25×7 + 18)."""
...
from grade_5_kanji import load_grade_5_kanji  # noqa: E402
...
def build_collection(part: int, entries: list) -> dict:
    ...
    return {
        ...
        "display": {
            "loop": False,
            "hideChrome": True,
            "family": "grade5StrokeOrder",
            "showKeyword": False,
            "showKanji": False,
            "showEnglish": False,
            "exhibitProfile": "grade5StrokeOrder",
        },
        "meta": {
            "series": SERIES_ID,
            "scope": SERIES_SCOPE,
            "part": part,
            "stage": "grade5StrokeOrder",
...
def main() -> int:
    ...
    parser.add_argument("--plan", action="store_true", help="Print batch plan for all 193 kanji")
    args = parser.parse_args()

    all_entries = load_grade_5_kanji()
    timing = dict(DEFAULT_EXHIBITION)
    batches = plan_batches(all_entries, timing)

    if args.plan:
        soundtrack_ms = probe_duration_ms(ROOT / soundtrack_path_for_part(1)) or 0
        print(f"Grade 5 stroke order batch plan ({len(all_entries)} kanji)")
        print(f"  layout: 25×7 + 18 → {PART_COUNT} parts")
        if soundtrack_ms:
