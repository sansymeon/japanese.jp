"""Grade 4 Stroke Order — timing, batching, and scene helpers."""
...
STROKE_OUTPUT_FADE_START_S = 9 * 60 + 50
...
SERIES_ID = "grade_4"
COLLECTION_PREFIX = "grade_4_strokes"
SERIES_SCOPE = "elementary_grade_4"
SERIES_TITLE = "Grade 4 Stroke Order"

# 202 kanji → 8 parts: 25×6 + 26×2 (parts 7–8 absorb the remainder).
KANJI_PER_PART = 25
PART_COUNT = 8
BOOKEND_IMAGE_COUNT = 8
# Parts 7–8 get one extra each so we stay at 8 bookends (no part 9).
PART_KANJI_COUNTS: dict[int, int] = {
    **{p: 25 for p in range(1, 7)},
    7: 26,
    8: 26,
}

SOUNDTRACK = "audio/grade_4_parts_1_3_minus3db.mp3"

_BOOKEND_IMAGE_BY_PART: dict[int, str] = {
    part: f"images/grade_4_stroke_orders_{part}.png"
...
def scene_for_entry(entry, *, part: int, index: int) -> dict:
    ...
    return {
        "id": f"G4S{part:02d}_{entry.slug}",
        "kanji": entry.kanji,
        "strokeOrder": stroke,
        "meta": {
            "part": part,
...
def plan_batches(entries: list, timing: dict) -> list[tuple[int, int, int]]:
    ...
    Parts 1–6: 25 kanji; parts 7–8: 26 kanji (202 total, 8 bookends).
    """
