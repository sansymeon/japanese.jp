#!/usr/bin/env python3
"""Record Grade 2 Compounds school edition MP4 via Playwright.

Output: collections/grade_2/grade_2_jukugo_{part}.mp4
"""
...
from grade2_compounds_school_common import PART_COUNT, collection_id  # noqa: E402
...
def record(*, part: int, port: int) -> Path:
    ...
    out_path = ROOT / "collections" / "grade_2" / f"grade_2_jukugo_{part}.mp4"
    tmp_dir = out_path.parent / f".tmp_grade_2_jukugo_{part}"
...
def main() -> int:
    ...
    if args.rebuild:
        subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_grade_2_compounds_school.py"), "--all"],
            check=True,
            cwd=ROOT,
        )
