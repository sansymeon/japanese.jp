#!/usr/bin/env python3
"""Calculate runtime for lesson_01-05_verses exhibition."""
...
COLLECTION = collection_json_path(ROOT, "lesson_01-05_verses")
...
def stage_ms(t: dict, prefix: str) -> int:
    return (
        t.get(f"{prefix}RevealMs", t.get("readingStageRevealMs", 1000))
        + t.get(f"{prefix}HoldMs", t.get("readingStageHoldMs", 7000))
        + t.get(f"{prefix}FadeMs", t.get("readingStageFadeMs", 1000))
    )


def exhibit_ms(t: dict, *, show_english: bool) -> int:
    stages = (
        stage_ms(t, "readingHiragana")
        + stage_ms(t, "readingMixed")
        + stage_ms(t, "readingNatural")
    )
    en = 0
    if show_english:
        en = (
            t.get("readingEnRevealMs", 1000)
            + t.get("readingEnHoldMs", 5000)
            + t.get("readingEnFadeMs", 1000)
        )
    return (
...
def main() -> int:
    if not COLLECTION.is_file():
        print(
            f"Missing {COLLECTION}. Run: python3 scripts/build_lesson_01_05_verses_exhibition.py",
            file=sys.stderr,
        )
        return 1
    ...
    show_english = display.get("showEnglish", True)

    per = exhibit_ms(t, show_english=show_english)
    body = per * len(scenes)
    ...
    print("Lessons 1–5 Verses exhibition runtime")
    print(f"  Collection:        {data.get('id')}")
    print(f"  Exhibit count:     {len(scenes)}")
    print(f"  Hiragana stage:    {stage_ms(t, 'readingHiragana') / 1000:.1f}s")
    print(f"  Mixed stage:       {stage_ms(t, 'readingMixed') / 1000:.1f}s")
    print(f"  Natural stage:     {stage_ms(t, 'readingNatural') / 1000:.1f}s")
    if show_english:
        en_ms = (
            t.get("readingEnRevealMs", 1000)
            + t.get("readingEnHoldMs", 5000)
        ...
        print(f"  English stage:     {en_ms / 1000:.1f}s")
