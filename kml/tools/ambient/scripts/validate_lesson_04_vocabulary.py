#!/usr/bin/env python3
"""Editorial QA for Lesson 4 Vocabulary Exhibition."""
...
REPO = ROOT.parents[1]
STEPS_MOD = Path(__file__).resolve().parent / "lesson_04_vocabulary_steps.py"
LESSON_HTML = REPO / "contents/books/book_01/lessons/lesson_04.html"
COLLECTION = ROOT / "collections/lesson_04/lesson_04_vocabulary.json"
...
def verses_by_slug() -> dict[str, str]:
    ...
    from build_lesson_04_vocabulary_exhibition import parse_lesson_scenes

    return {
        s["meta"]["slug"]: s["verse"]["jpHtml"]
        for s in parse_lesson_scenes(4)
    }
...
def main() -> int:
    ...
    all_errors: list[str] = []
    all_errors.extend(errors_for_html(lesson_html, label="lesson_04.html"))
