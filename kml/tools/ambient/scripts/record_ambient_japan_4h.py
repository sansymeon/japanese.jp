#!/usr/bin/env python3
"""Record Ambient Japan 4h MP4 via Playwright (full 142-image film).

Usage:
  python scripts/record_ambient_japan_4h.py
  python scripts/record_ambient_japan_4h.py --rebuild

Output: collections/ambient_japan_4h_scenery/ambient_japan_4h.mp4

Overnight: capture is ~4h 1min wall clock, then h264 mux (~several hours).
"""
...
OUTPUT_NAME = "ambient_japan_4h.mp4"
DEFAULT_PORT = 9088
...
def record(*, port: int, rebuild: bool) -> Path:
    if rebuild:
        subprocess.check_call(
            [sys.executable, str(ROOT / "scripts" / "build_ambient_japan_4h.py")],
            cwd=str(ROOT),
        )

    collection = load_collection(ROOT, COLLECTION_ID)
    soundtrack_start_ms = vocabulary_exhibition_soundtrack_start_ms(collection)
    timeout_ms = presentation_timeout_ms(collection, ROOT, extra_ms=1_800_000)
    ...
    tmp_dir = out_dir / ".tmp_ambient_japan_4h"
    ...
    print(f"Recording {COLLECTION_ID} → {out_path}", flush=True)
    print(f"  Scenes: {meta.get('sceneCount')}", flush=True)
    print(f"  Hold: {meta.get('holdMs')} ms", flush=True)
    print(f"  Soundtrack: {soundtrack_rel}", flush=True)
    print(f"  Soundtrack @ {soundtrack_start_ms} ms", flush=True)
    print(
        f"  Max wait: {timeout_ms // 1000}s ({timeout_ms / 3600000:.2f} h)",
        flush=True,
    )
    print(f"  URL: {url}", flush=True)
    print("  Chromium: long-capture flags; record badge hidden", flush=True)
